...
def execute_job(SDE_CONNECTION,SDE_FEATURE_CLASS_NAME,TEMP_WORKING_DIRECTORY,SLIP_DATASET_NAME,AWS_CREDENTIAL_PROFILE,AWS_S3_BUCKET,AWS_S3_FOLDER_KEY):
    try:
        ...
        s3_uploadURL = UploadFile(my_temp_zip,AWS_S3_BUCKET,AWS_S3_FOLDER_KEY+os.path.basename(my_temp_zip),AWS_CREDENTIAL_PROFILE)
        logging.info('Successfully Uploading [%s] to S3' % s3_uploadURL)
...
def main(argv):
    ...
    if (SDE_CONNECTION == '' or SDE_FEATURE_CLASS_NAME == '' or SLIP_DATASET_NAME == '' or AWS_CREDENTIAL_PROFILE == '' or TEMP_WORKING_DIRECTORY == '' or AWS_S3_BUCKET == '' or AWS_S3_FOLDER_KEY == ''):
